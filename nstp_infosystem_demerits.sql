...
CREATE TABLE `demerits` (
  `demerit_id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  KEY `fk_dem_student` (`student_id`),
  CONSTRAINT `fk_dem_student` FOREIGN KEY (`student_id`) REFERENCES `students` (`student_id`) ON DELETE CASCADE ON UPDATE CASCADE
